...
//		System.out.println("start recipesService.test.TestServer");
		...
		try {
			...
			listeningPort =Integer.parseInt(properties.getProperty("testServerPort"));
			if (argsList.contains("-p")){
				int i = argsList.indexOf("-p");
				listeningPort = Integer.parseInt(args[i+1]);
			} 

			System.out.println("start recipesService.test.TestServer (on port: "+listeningPort+")");
			...
			System.err.println("\t-p <port>: listening port of TestServer");
			...
			System.err.println("\t-phase <numPhase>: phase to be tested (default: all)");
